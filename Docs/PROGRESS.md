...
- Validating the new app shell against live hardware parity and replacing the remaining provisional false-color route with a verified `/v2` operation if one exists.
...
- Resolve the false-color control path, or explicitly gate the UI on unsupported firmware.
